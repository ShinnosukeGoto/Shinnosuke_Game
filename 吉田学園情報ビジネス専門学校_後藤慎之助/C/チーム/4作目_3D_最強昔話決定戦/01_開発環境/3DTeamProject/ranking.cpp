...
void InitRanking(void)
{
    ...
    if (menu == MENU_GAME)
    {
        g_nGameScore = GetScore();
    }

    //ランキングから遷移したなら
    else if (menu == MENU_RANKING)
    {
        g_nGameScore = 0;
    }
...
void UpdateRanking(void)
{
    ...
    if (nFade == FADE_NONE)
    {
            ...
            || GetJoypadTrigger(PLAYER_1, BUTTON_B) || GetJoypadTrigger(PLAYER_1, BUTTON_START))
        {
            ...
            if (menu == MENU_GAME)
            {
                StopSound(SOUND_LABEL_BGM_RESULT);	//リザルトのBGMを止める
                SetFade(MODE_TITLE);//タイトル画面に移動
            }
            else if (menu == MENU_RANKING)
            {
                SetFade(MODE_MENU); //メニュー画面に移動
            }
...
void DrawRanking(void)
{
    ...
    //スコアの数字の描画
    for (int nResultScore = 1; nResultScore < 200; nResultScore++)	//pVtx0～3を読み込まないようにするため、1からforを始める
    {
        //テクスチャの設定
...
void ReadScore(void)
{
    ...
    g_nRankingScore[RANKING_SCORE_MAX - 1] = g_nGameScore;
